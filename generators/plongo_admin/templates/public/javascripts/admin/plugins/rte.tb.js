...
function lwrte_color(){
	...
	var panel = self.create_panel('Set color for text', '\
...
<p class="submit"><button id="ok">Ok</button><button id="cancel">Cancel</button></p>');
	var panel_dom = panel.boxy;

	var preview = $('#preview', panel_dom);
	var color = $("#color", panel_dom);
	var palette = $("#palette", panel_dom);
	var colors = [
	...
	$('#cancel', panel_dom).click( function() { panel_dom.hide(); return false; } );
	$('#ok', panel_dom).click( 
		function() {
...
function lwrte_image() {
	...
	var panel = self.create_panel("Insert image", '<p class="loader"><img src="/images/admin/plugins/indicator.gif" /></p>');
	
	// assets
	jQuery.getJSON('/admin/image_assets.json', {}, function(data) {
		var assets = data.assets;	
		var html = '<ul id="pick-assets">';
		for (var i = 0; i < assets.length; i++) {
			var asset = assets[i];
			html += '<li><a href="' + asset.url + '">' + asset.name + '<em>' + asset.size + '</em></a></li>'
		}
		html += '</ul><p class="help">Click on an asset to insert it</p>';
		
		panel.setContent(html);
		
		$('#pick-assets li a', panel.boxy).click(function(e) {
			self.editor_cmd('insertImage', $(this).attr('href'));
			e.preventDefault();
			e.stopPropagation();
			panel.hide();
			return false;
...
function lwrte_link() {
	...
	var panel = self.create_panel("Create link / Attach file", '\
<p class="inputs"><label>URL</label><input type="text" id="url" size="30" value=""><button id="file">Attach File</button><button id="view">View</button></p>\
<div class="clear"></div>\
<p class="inputs"><label>Title</label><input type="text" id="title" size="30" value=""><label>Target</label><select id="target"><option value="">default</option><option value="_blank">new</option></select></p>\
<div class="clear"></div>\
<p class="submit"><button id="ok">Ok</button></p>');
	var panel_dom = panel.boxy;

	var url = $('#url', panel_dom);
	
	if (self.get_selected_html() && self.get_selected_html().length > 0) {
		var matches = self.get_selected_html().match(new RegExp('href=\"([^\"]*)\".*\>'));
		if (matches && matches.length > 0) url.val(matches[1]);
		
		matches = self.get_selected_html().match(new RegExp('title=\"([^\"]*)\".*\>'));
		if (matches && matches.length > 0) $('#title', panel_dom).val(matches[1]);
	}
	
	var upload = $('#file', panel_dom).upload({
		name: 'asset[source]',
		autoSubmit: true,
		action: '/admin/assets',
		onComplete: function(response) {
			...
			if(response.error && response.error.length > 0)
				...
				url.val((response.url && response.url.length > 0) ? response.url : '');
		},
		params: JsonData
	});

	$('#view', panel_dom).click( function() {
		(url.val().length >0 ) ? window.open(url.val()) : alert("Enter URL to view");
		return false;
	});

	$('#ok', panel_dom).click(function() {
		var url = $('#url', panel_dom).val();
		var target = $('#target', panel_dom).val();
		var title = $('#title', panel_dom).val();
		...
		var iframe_body = self.iframe.contentWindow.document.body;

		if(target.length > 0)
			$('a[href*="' + rte_tag + '"]', iframe_body).attr('target', target);
		
		if(title.length > 0)
			$('a[href*="' + rte_tag + '"]', iframe_body).attr('title', title);
		
		$('a[href*="' + rte_tag + '"]', iframe_body).attr('href', url);
